...
class InfluenceInvariants(object):
    ...
    def compile(self):
        ...
        self._attrib_shapes = self._attributer.get_attributions(zero_input).shape[1:]
    ...
    def get_invariants(self, x, min_support=None, min_precision=1.0, batch_size=None, **kwargs):
        ...
        while len(stack) > 0:
            ...
            if(clf.tree_.children_left[node] != clf.tree_.children_right[node]):
                # _, _, unit = map_feat_to_layer(clf.tree_.feature[node])
                unit = clf.tree_.feature[node]
                au = self._attributer.symbolic_attributions[:,unit]
                lit_f = Literal(self.layer, unit, K.less_equal, 0, attribution_unit=au)
                lit_t = Literal(self.layer, unit, K.greater, 0, attribution_unit=au)
                ...
                if (min_support is None or min_support <= support) and (min_precision is None or min_precision <= precision):
                    invs.append(Invariant([clause], self._attributer.model, Q=q, support=support, precision=precision))
